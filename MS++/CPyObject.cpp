...
int CPyObject::Check3(int val) const
{
	if (val == -1)
		CheckErr();
...
CPyException::CPyException(const CPyString &text)
{
	m_text = text;
}
...
{
	if (!PyIter_Check(m_obj))
		throw CPyException("not an iterator");
...
CPyFloat::CPyFloat(PyObject *obj) : CPyObject(obj)
{
	if (PyLong_Check(m_obj))
		m_obj = CheckObj(PyFloat_FromDouble(PyLong_AsDouble(m_obj)));

	if (!IsFloat())
		throw CPyException("not a float");
}
